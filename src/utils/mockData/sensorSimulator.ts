...
/**
 * Sensor Simulator for development
 *
 * Provides keyboard control over a mocked DeviceOrientation when
 * NEXT_PUBLIC_USE_MOCK_SENSORS (or window.__USE_MOCK_SENSORS) is enabled.
 *
 * Controls (deg per keypress):
 * - Left/Right or A/D: alpha (azimuth/compass heading) -- wraps 0..360
 * - Up/Down or W/S: beta (altitude tilt front/back) -- clamped -90..90
 * - Q/E: gamma (roll tilt left/right) -- clamped -90..90
 * Hold Shift to step x3.
 */

type MockOrientation = {
  alpha: number
  beta: number
  gamma: number
}

export function useMockOrientation(initial: Partial<MockOrientation> = {}) {
  const envEnabled =
    ...
    (typeof window !== 'undefined' && (window as any).__USE_MOCK_SENSORS === true)

  const [enabled] = useState<boolean>(!!envEnabled)
  const [alpha, setAlpha] = useState<number>(initial.alpha ?? 0)
  const [beta, setBeta] = useState<number>(initial.beta ?? 0)
  const [gamma, setGamma] = useState<number>(initial.gamma ?? 0)
  ...
  const stepAlpha = useCallback((delta: number) => setAlpha((a) => wrap360(a + delta)), [])
  const stepBeta = useCallback((delta: number) => setBeta((b) => clamp(b + delta, -90, 90)), [])
  const stepGamma = useCallback((delta: number) => setGamma((g) => clamp(g + delta, -90, 90)), [])
  ...
  // Log basic human-friendly direction for quick feedback
  useEffect(() => {
    if (!enabled) return
    logDirection(alpha, beta, gamma)
  }, [enabled, alpha, beta, gamma])

  // Expose helpers in dev window for quick calls
  useEffect(() => {
    if (!enabled || typeof window === 'undefined') return
    ;(window as any).MOCK_ORI = {
      get: () => ({ alpha, beta, gamma }),
      lookNorth, lookSouth, lookEast, lookWest, lookUp,
      set: (a: number, b: number, g: number) => { setAlpha(a); setBeta(b); setGamma(g) },
    }
...
function humanDirection(alpha: number, beta: number): string {
...
function logDirection(alpha: number, beta: number, gamma: number) {
